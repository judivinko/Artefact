...
// jedinstveni ID svake knjige
function generatePdfId() {
  return "pdf_" + Date.now() + "_" + Math.floor(Math.random()*999999);
}

// spremanje PDF-a u /public/pdfs
function savePdf(file, newName) {
  const dir = path.join(__dirname, "public", "pdfs");
  fs.mkdirSync(dir, { recursive: true });

  const full = path.join(dir, newName);
  fs.writeFileSync(full, file.buffer); // file.buffer dobijaš preko multer-a
  return "/pdfs/" + newName; // putanja za frontend
}
...
app.post("/api/transfer-gold", (req, res) => {
  try {
    ...
    if (!(g > 0)) return res.status(400).json({ ok:false, error:"Gold must be > 0" });
    ...
    const out = db.transaction(() => {
      ...
      if (!s || (s.balance_silver|0) < deltaS) throw new Error("Insufficient funds");
      ...
      const after = db.prepare("SELECT balance_silver FROM users WHERE id=?").get(senderId).balance_silver;
...
app.get("/api/daily/status", (req, res) => {
  try {
    ...
    const lastClaim = row.last_claim;
    ...
    // === Da li je sljedeći dan zaključan?
    let locked = false;
    let secondsLeft = 0;
