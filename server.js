// === ARTEFAKT — Economy V3 (Shop + Recipes + Craft + Auctions + Admin) ===
...
// --- ENV / CONFIG (Render-friendly) ---
const PORT = process.env.PORT || 3000;
const HOST = "0.0.0.0";
const JWT_SECRET = process.env.JWT_SECRET || "dev-secret-change-me";
const TOKEN_NAME = "token";
const ADMIN_KEY = process.env.ADMIN_KEY || "dev-admin-key";
const COOKIE_SECURE = (process.env.COOKIE_SECURE === "true"); // true on Render
const DB_PATH = process.env.DB_PATH || path.join(__dirname, "rps.db"); // e.g. /var/data/rps.db on Render
...
function isAdminRequest(req){
  ...
  const u=verifyTokenFromCookies(req); if(!u) return false;
...
try{
  // Migrate old gold -> silver if present
...
ensureColumn("recipes","retired INTEGER NOT NULL DEFAULT 0");
...
function ensureRecipe(code,name,tier,outCode,ings){
  ...
  let rid;
  if(!r){
    db.prepare("INSERT INTO recipes(code,name,tier,output_item_id,retired) VALUES (?,?,?,?,0)").run(code,name,tier,outId);
    rid=db.prepare("SELECT id FROM recipes WHERE code=?").get(code).id;
  }else{
    db.prepare("UPDATE recipes SET name=?,tier=?,output_item_id=?,retired=0 WHERE id=?").run(name,tier,outId,r.id);
    rid=r.id;
...
// --- Items/Recipes (minimal demo set; names in English) ---
// SCRAP (volatile)
...
ensureItem("SMP","Resin Coating",2,0);
...
ensureRecipe("R_SMP","Resin Coating",2,"SMP",[["SMO",1],["SMO",1],["DRV",1]]);
...
ensureItem("PRC","Precise Tool",4,0);

// T4 recipes
ensureRecipe("R_LMP","Lamp",4,"LMP",[["STB",1],["OKA",1],["BRP",1]]);
ensureRecipe("R_PRC","Precise Tool",4,"PRC",[["BRN",1],["KLP",1],["PAP",1]]);
...
// --- Boot cleanup: wipe volatile scrap from inventories & purge retired/unknown recipes from users ---
try{
  const volIds=db.prepare("SELECT id FROM items WHERE volatile=1").all().map(r=>r.id);
  if(volIds.length){
    db.prepare(`DELETE FROM user_items WHERE item_id IN (${volIds.map(()=>"?").join(",")})`).run(...volIds);
  }
  // Remove user_recipes pointing to retired or missing recipes
  db.prepare(`
    DELETE FROM user_recipes
    WHERE recipe_id NOT IN (SELECT id FROM recipes WHERE retired=0)
  `).run();
}catch{}
...
app.get("/api/me",(req,res)=>{
  ...
  const buysToNext=(r.next_recipe_at==null)?null:Math.max(0,r.next_recipe_at-r.shop_buy_count);
...
function grantRandomRecipeIfDue(userId){
  ...
  if(u.next_recipe_at==null){
    db.prepare("UPDATE users SET next_recipe_at=? WHERE id=?").run(u.shop_buy_count+nextInterval(),userId);
    return {granted:false};
  }
  if(u.shop_buy_count < u.next_recipe_at) return {granted:false};

  // allow duplicates; take all recipes where not retired
  const list=db.prepare(`SELECT id,code,name,tier FROM recipes WHERE retired=0`).all();
  if(!list.length){
    db.prepare("UPDATE users SET next_recipe_at=? WHERE id=?").run(u.shop_buy_count+nextInterval(),userId);
    return {granted:false};
  }
...
app.post("/api/shop/buy-t1",(req,res)=>{
  ...
  if(!uTok) return res.status(401).json({ok:false,error:"Not signed in."});
  try{
...
app.get("/api/my/recipes",(req,res)=>{
  ...
  const rows=db.prepare(`
    ...
    WHERE ur.user_id=? AND ur.qty>0 AND r.retired=0
...
app.get("/api/recipes/:id",(req,res)=>{
  ...
  const rec=db.prepare(`
    ...
    WHERE r.id=? AND r.retired=0
  `).get(u.uid,rid);
  if(!rec || !rec.have_qty) return res.status(404).json({ok:false,error:"You don't have this recipe."});
...
app.post("/api/recipes/:id/craft",(req,res)=>{
  const u=verifyTokenFromCookies(req); if(!u) return res.status(401).json({ok:false,error:"Not signed in."});
  const rid=parseInt(req.params.id,10);
  try{
    const out=db.transaction(()=>{
      const rec=db.prepare(`
        ...
        WHERE r.id=? AND r.retired=0
      `).get(u.uid,rid);
      if(!rec||!rec.have_qty) throw new Error("You don't have this recipe.");
      ...
      for(const ing of ings){ if(ing.have_qty<ing.need_qty) throw new Error(`Missing: ${ing.name} x${ing.need_qty-ing.have_qty}`); }
...
app.get("/api/my/inventory",(req,res)=>{
  ...
  const recipes=db.prepare(`
    ...
    WHERE ur.qty>0 AND r.retired=0 ORDER BY r.tier ASC,r.name ASC
  `).all(u.uid);
  res.json({ok:true,items,recipes});
});

// --- Helper to find item/recipe by code ---
function findItemOrRecipeByCode(code){
  if(!code || typeof code!=="string") return null;
  if(code.startsWith("R_")){
    const r=db.prepare("SELECT id,code,name,tier FROM recipes WHERE code=? AND retired=0").get(code);
...
app.post("/api/auctions/create",(req,res)=>{
  ...
  if(sBuy && sBuy<sStart) return res.status(400).json({ok:false,error:"Buy-now cannot be less than start price."});
  ...
  try{
    const result=db.transaction(()=>{
      if(look.kind==="item"){
        ...
        if(!r||r.qty<q) throw new Error("Not enough quantity.");
...
app.post("/api/auctions/:id/bid",(req,res)=>{
  ...
  try{
    const result=db.transaction(()=>{
      ...
      if(new Date()>new Date(a.end_time)) throw new Error("Auction ended.");
      const me=db.prepare("SELECT id,balance_silver FROM users WHERE id=?").get(u.uid);
      if(u.uid===a.seller_user_id) throw new Error("Cannot bid on your own auction.");
...
app.post("/api/auctions/:id/buy-now",(req,res)=>{
  ...
  try{
    const result=db.transaction(()=>{
      ...
      if(u.uid===a.seller_user_id) throw new Error("Cannot buy your own auction.");
...
// --- ADMIN (extra used by admin.html) ---
// Rescue: make admin
app.post("/api/admin/make-admin", (req,res)=>{
  ...
  res.json({ok:true,message:"User is now admin."});
});
...
// Hard delete user (only if no active auctions/holds)
app.post("/api/admin/hard-delete-user",(req,res)=>{
  ...
  try{
    const tx=db.transaction(()=>{
      const liveA=db.prepare("SELECT COUNT(*) c FROM auctions WHERE seller_user_id=? AND status='live'").get(u.id).c|0;
      const holds=db.prepare(`
        SELECT COUNT(*) c FROM money_holds h
        JOIN auctions a ON a.id=h.auction_id
        WHERE a.seller_user_id=? OR h.user_id=?
      `).get(u.id,u.id).c|0;
      if(liveA>0 || holds>0) throw new Error("User has live auctions or holds.");
...
// Wipe economy (close auctions, refund holds, return escrow, clear inventories, zero balances)
  ...
  try{
    const tx=db.transaction(()=>{
      ...
      for(const a of live){
        ...
        if(esc){
          if(esc.item_id){
            db.prepare(`INSERT INTO user_items(user_id,item_id,qty) VALUES (?,?,?) ON CONFLICT(user_id,item_id) DO UPDATE SET qty=qty+excluded.qty`)
              .run(esc.owner_user_id,esc.item_id,esc.qty);
          }else if(esc.recipe_id){
            db.prepare(`INSERT INTO user_recipes(user_id,recipe_id,qty) VALUES (?,?,?) ON CONFLICT(user_id,recipe_id) DO UPDATE SET qty=qty+excluded.qty`)
              .run(esc.owner_user_id,esc.recipe_id,esc.qty);
          }
          db.prepare("DELETE FROM inventory_escrow WHERE auction_id=?").run(a.id);
        }
        db.prepare("UPDATE auctions SET status='canceled' WHERE id=?").run(a.id);
      }
      // clean holds / escrows (safety)
      db.prepare("DELETE FROM money_holds").run();
      db.prepare("DELETE FROM inventory_escrow").run();

      // clear inventories & trophies
      db.prepare("DELETE FROM user_items").run();
      db.prepare("DELETE FROM user_recipes").run();
      db.prepare("DELETE FROM user_trophies").run();

      // zero balances and counters
      db.prepare("UPDATE users SET balance_silver=0, shop_buy_count=0, next_recipe_at=NULL").run();

      // optional: keep auctions history & bids for analytics; not deleting past 'paid'/'canceled'
    });
    tx();
...
// Admin: list users (active first, then alphabetical)
app.get("/api/admin/users",(req,res)=>{
  if(!isAdminRequest(req)) return res.status(401).json({ok:false,error:"Unauthorized"});
  const rows=db.prepare(`
    SELECT
      id,email,is_admin,is_disabled,
      CAST(balance_silver/100 AS INT) as gold,
      (balance_silver%100) as silver,
...
// Admin: mark recipes retired / unretire
app.post("/api/admin/retire-recipes",(req,res)=>{
  if(!isAdminRequest(req)) return res.status(401).json({ok:false,error:"Unauthorized"});
  const {codes=[], retired=true}=req.body||{};
  if(!Array.isArray(codes) || codes.length===0) return res.status(400).json({ok:false,error:"Provide codes[]"});
  const tx=db.transaction(()=>{
    for(const code of codes){
      db.prepare("UPDATE recipes SET retired=? WHERE code=?").run(retired?1:0, String(code||"").trim());
    }
  });
  try{ tx(); res.json({ok:true}); }catch(e){ res.status(400).json({ok:false,error:String(e.message||e)}); }
});
// Admin: purge retired/missing from user inventories
app.post("/api/admin/purge-retired",(req,res)=>{
  if(!isAdminRequest(req)) return res.status(401).json({ok:false,error:"Unauthorized"});
  db.prepare(`
    DELETE FROM user_recipes
    WHERE recipe_id NOT IN (SELECT id FROM recipes WHERE retired=0)
...
// --- Health ---
app.get("/api/health",(req,res)=> res.json({ ok:true, msg:"Shop, Recipes, Craft & Auctions ready", fee_bps:AUCTION_FEE_BPS, db:DB_PATH, secureCookies:COOKIE_SECURE }));
io.on("connection", s => s.emit("hello",{ok:true,msg:"artefakt econ v3"}));
